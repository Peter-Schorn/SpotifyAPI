...
extension Playlist: Codable {
    ...
    public init(
        ...
        self.images = try container.decodeIfPresent(
            [SpotifyImage?].self,
            forKey: Playlist<Items>.CodingKeys.images
        )?.compactMap({ $0 }) ?? []
        ...
        self.name = try container.decode(
            String.self, 
            forKey: Playlist<Items>.CodingKeys.name
        )

        self.items = try container.decode(
            Items.self, 
            forKey: Playlist<Items>.CodingKeys.items
        )

        self.owner = try container.decodeIfPresent(
            SpotifyUser.self, 
            forKey: Playlist<Items>.CodingKeys.owner
        )

        self.isPublic = try container.decodeIfPresent(
            Bool.self, 
            forKey: Playlist<Items>.CodingKeys.isPublic
        )

        self.isCollaborative = try container.decode(
            Bool.self, 
            forKey: Playlist<Items>.CodingKeys.isCollaborative
        )

        self.description = try container.decodeIfPresent(
            String.self, 
            forKey: Playlist<Items>.CodingKeys.description
        )

        self.snapshotId = try container.decode(
            String.self, 
            forKey: Playlist<Items>.CodingKeys.snapshotId
        )

        self.externalURLs = try container.decodeIfPresent(
            [String : URL].self, 
            forKey: Playlist<Items>.CodingKeys.externalURLs
        )

        self.followers = try container.decodeIfPresent(
            Followers.self, 
            forKey: Playlist<Items>.CodingKeys.followers
        )

        self.href = try container.decode(
            URL.self, 
            forKey: Playlist<Items>.CodingKeys.href
        )

        self.id = try container.decode(
            String.self, 
            forKey: Playlist<Items>.CodingKeys.id
        )

        self.uri = try container.decode(
            String.self, 
            forKey: Playlist<Items>.CodingKeys.uri
        )

        self.type = try container.decode(
            IDCategory.self, 
            forKey: Playlist<Items>.CodingKeys.type
        )
