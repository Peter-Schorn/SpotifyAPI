...
public extension SpotifyAPI {
    ...
    /**
     ...
     - Parameters:
       ...
       - locale: *Optional*. The desired language, consisting of an
             [ISO 639-1][3] language code and an
             [ISO 3166-1 alpha-2 country code][2], joined by an underscore.
             For example: es_MX, meaning "Spanish (Mexico)". Provide this
             parameter if you want the category strings returned in a
             particular language. Note that, if this paramater is not
             supplied, or if the specified language is not available,
    ...
    /**
     ...
     See also `category(_:country:locale:)` (gets a single category based on an id).
     ...
     - Parameters:
       - country: *Optional*. A country: an [ISO 3166-1 alpha-2 country code][2].
             Provide this parameter if you want to narrow the list of returned
             categories to those relevant to a particular country. If omitted,
             the returned items will be globally relevant.
       - locale: *Optional*. The desired language, consisting of an
             [ISO 639-1 language code][3] and an [ISO 3166-1 alpha-2 country code][2],
             ...
             specified language is not available, all strings will be returned in
             the Spotify default language (American English). The locale parameter,
             ...
             `locale` will return a list of categories relevant to Sweden but as
    ...
    /**
     ...
     - Parameters:
       - id: A category id.
       - country: *Optional*. A country: an
             [ISO 3166-1 alpha-2 country code][2].
    ...
    /**
     Get a list of featured playlists (shown, for example, on a
     ...
     - Parameters:
       ...
       - timestamp: *Optional*. A date, which will be converted to a
             second-precision timestamp ("yyyy-MM-ddTHH:mm:ss"). Use this parameter
             to specify the user’s local time to get results tailored for that
             specific date and time in the day. If not provided, the response defaults
             to the current UTC time. If there were no featured playlists (or there is
             no data) at the specified time, the response will revert to the current
             UTC time.
    ...
    /**
     ...
     - Parameters:
       - country: *Optional*. A country: an [ISO 3166-1 alpha-2 country code][2].
             Provide this parameter if you want the list of returned albums to be
    ...
    /**
     ...
     Create a playlist-style listening experience based on seed artists,
     tracks and genres.
     ...
     - Parameters:
       ...
       - limit: *Optional*. The target size of the list of recommended tracks.
             ...
             number of recommended tracks. Debugging information for such cases is
             available in the response. Default: 20; Minimum: 1; Maximum: 100.
       - market: *Optional*. [An ISO 3166-1 alpha-2 country code][3] or the string
             "from_token". Provide this parameter if you want to apply
             [Track Relinking][4]. Because minimum, maximum, and target values
             are applied to pools before relinking, the generated results may not
             precisely match the filters applied. Original, non-relinked tracks are
             available via the `linkedFrom` attribute of the
    ...
    /**
     ...
     Thesse values can be used in the `seedGenres` property of
